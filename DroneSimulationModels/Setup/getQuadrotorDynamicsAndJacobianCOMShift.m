...
IxxVal = 0.000058286*0.5; 
IyyVal = 0.000071691*0.6;
IzzVal = 0.0001;
mVal = 0.063;
kVal = 0.0107;
lVal = 0.0624;
...
matlabFunction(transpose(f),'File','QuadrotorStateFcnCOMShift',...
    'Vars',{transpose([state{:}]),transpose(control)});
% Create QuadrotorStateJacobianFcn.m 
matlabFunction(A, B,'File','QuadrotorStateJacobianFcnCOMShift',...
    'Vars',{transpose([state{:}]),transpose(control)});
