...
tau_beta = [l*k*(-u2 + u4);l*k*(-u1 + 0.9*u3);b*(-u1+u2-u3+u4)];
...
matlabFunction(transpose(f),'File','QuadrotorStateFcnBladeDamage',...
    'Vars',{transpose([state{:}]),transpose(control)});
% Create QuadrotorStateJacobianFcn.m 
matlabFunction(A, B,'File','QuadrotorStateJacobianFcnBladeDamage',...
    'Vars',{transpose([state{:}]),transpose(control)});
